...
const sampleJWT = `
eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IlRhcm8gWWFtZGEiLCJpYXQiOjE1MTYyMzkwMjJ9.y2fzIzmjE174WN66kLftCaoqBWxJOYgEZafbly2x-UY`.trim()

const base64ToStr = (s: string): string => CryptoJS.enc.Base64.parse(s).toString(CryptoJS.enc.Utf8)

const decodeJWT = (token: string) => {
  const [p1, p2, _p3] = token.split('.').map((e) => {
    const base64 = e.replace(/-/g, '+').replace(/_/g, '/')
    return base64
  })
  return {
